...
### USAGE ##########################################################################################
# haplotype_caller.pl -t tool_config.yaml -c data_config.yaml --dna { or --rna }
#
# where:
...
# 	or STAR-aligned BAMs (post gatk processing), generated by create_final_yaml.pl)
...
sub main{
	...
	# start logging
	print "---\n";
	print "Running Variant Call (HaplotypeCaller) pipeline.\n";
	...
	if ('dna' eq $data_type) {
		...
		# check if this should be run
		if ( ('N' eq $resume) || ('Y' eq missing_file($combined_gvcf . '.md5')) ) {

			# record command (in log directory) and then run job
			print "Submitting job for CombineGVCFs...\n";
			...
			print "Skipping CombineGVCFs because this has already been completed!\n";
			}
	...
	if ( ('rna' eq $data_type) && ('Y' eq $tool_data->{create_output_yaml}) ) {
