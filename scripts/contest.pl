...
### USAGE ##########################################################################################
# contest.pl -t tool.yaml -d data.yaml -o /path/to/output/dir -h slurm -r Y -n Y
#
# where:
	...
	foreach my $patient (sort keys %{$smp_data}) {
		...
		foreach my $sample (@tumour_ids) {
			...
			foreach my $lane_type (@lane_types) {
				...
				my $output_file = join('/', $patient_directory, $sample . '_ContEst_' . $lane_type . '.tsv');
				...
				my $contest_command = get_contest_command(
					...
					java_mem	=> $tool_data->{parameters}->{contest}->{java_mem},
				...
				if ('Y' eq missing_file($output_file . '.md5')) {
					...
					print $log "Skipping ContEst ($lane_type) because this has already been completed!\n";
					}
