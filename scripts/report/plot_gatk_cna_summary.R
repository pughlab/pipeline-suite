...
parser$add_argument('-m', '--metrics', type = 'character',
	help = 'path to pga estimates (generated by collect_gatk_cnv_output.R)');
...
if (is.null(arguments$cnas)) {
	stop('ERROR: No CNA calls provided, please provide path to CNA matrix from GATK:CNV.');
	} else {
	input.data <- read.delim(arguments$cnas, stringsAsFactors = FALSE);
	}

if (is.null(arguments$metrics)) {
	stop('ERROR: No PGA file provided, please provide path to metrics file from GATK:CNV.');
	} else {
...
if (!is.null(arguments$report)) {
	...
	tex.file <- paste0(
		arguments$report,
		'/cna_summary_gatk.tex'
		);

	# create symlinks for plots
	unlink(paste0(arguments$report,'/gatk_scna_landscape.png'));
	file.symlink(
		paste0(arguments$output,'/',
			generate.filename(arguments$project, 'gatk_scna_landscape','png')),
		paste0(arguments$report,'/gatk_scna_landscape.png')
		);
	...
	write("Copy number variation was also called using GATK (v4.1.8.1) using GATK's best practices guidelines:", file = tex.file, append = TRUE);
