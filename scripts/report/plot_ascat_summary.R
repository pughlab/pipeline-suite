...
# INPUT:
#	- copy number calls and ploidy/purity estimates (output by collect_ascat_output.R)
...
parser$add_argument('-c', '--cnas', type = 'character', help = 'path to gene by sample matrix of cna calls (generated by collect_ascat_output.R)');
parser$add_argument('-m', '--metrics', type = 'character', help = 'path to ploidy and cellularity estimates (generated by collect_ascat_output.R)');
...
#	colour.alpha = if (arguments$scale == 'CN') { 1 } else { 0.5 },
	...
	resolution = 1600,
	right.padding = 12,
	filename = generate.filename(arguments$project, 'ascat_cna_landscape','png')
	);
...
create.multipanelplot(
	...
	width = 8,
	resolution = 200,
	filename = generate.filename(arguments$project, 'ascat_metrics','png'),
	layout.width = 3,
	layout.height = 1,
	plot.objects.widths = if (axis.cex > 0) { c(1.4,1,1) } else { c(1.2,1,1) },
...
save(
	cna.data,
	ploidy.estimates,
	file = generate.filename(arguments$project, 'ascat_data', 'RData')
	);
...
if (arguments$scale == 'CN') {
	...
	cna.caption <- "Heatmap shows log$_2$(ratio) from ASCAT; ordered by genomic position (chr1 [left] to chrX [right]) and PGA (high [top] to low [bottom]); only available for tumour samples with a matched normal.";
	}

# write for latex
write("\\section{SCNA Summary}", file = 'ascat_summary.tex');
write("Copy number variation calls from ASCAT (v3.1.0) with optimized gamma parameter:", file = 'ascat_summary.tex', append = TRUE);

# CNA landscape plot
write("\\begin{figure}[h!]", file = 'ascat_summary.tex', append = TRUE);
write("\\begin{center}", file = 'ascat_summary.tex', append = TRUE);
write(paste0(
	"\\includegraphics[width=0.9\\textwidth]{",
	getwd(), '/',
	generate.filename(arguments$project, 'ascat_cna_landscape','png'), '}'
	), file = 'ascat_summary.tex', append = TRUE);
write("\\end{center}", file = 'ascat_summary.tex', append = TRUE);
write(paste0(
	"\\caption{", cna.caption, "}"
	), file = 'ascat_summary.tex', append = TRUE);
write("\\end{figure}\n", file = 'ascat_summary.tex', append = TRUE);
...
if ((nrow(ploidy.estimates) <= 20)) {
	write("\n\\begin{figure}[h!]", file = 'ascat_summary.tex', append = TRUE);
	write("\\begin{center}", file = 'ascat_summary.tex', append = TRUE);
	write(paste0(
		"\\includegraphics[width=0.5\\textwidth]{",
		getwd(), '/',
		generate.filename(arguments$project, 'ascat_metrics','png'), '}'
		), file = 'ascat_summary.tex', append = TRUE);
	} else {
	write("\\pagebreak\n\\begin{figure}[h!]", file = 'ascat_summary.tex', append = TRUE);
	write("\\begin{center}", file = 'ascat_summary.tex', append = TRUE);
...
### SAVE SESSION INFO ##############################################################################
save.session.profile(generate.filename('ASCATSummary','SessionProfile','txt'));
