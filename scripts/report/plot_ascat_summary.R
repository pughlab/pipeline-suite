...
parser$add_argument('-c', '--cnas', type = 'character',
	help = 'path to gene by sample matrix of cna calls (generated by collect_ascat_output.R)');
parser$add_argument('-m', '--metrics', type = 'character',
	help = 'path to ploidy and cellularity estimates (generated by collect_ascat_output.R)');
...
if (is.null(arguments$cnas)) {
	stop('ERROR: No CNA calls provided, please provide path to CNA matrix from ASCAT.');
	} else {
	input.data <- read.delim(arguments$cnas, stringsAsFactors = FALSE);
	}

if (is.null(arguments$metrics)) {
	stop('ERROR: No CNA metrics provided, please provide path to metrics file from ASCAT.');
	} else {
...
if (!is.null(arguments$report)) {
	...
	tex.file <- paste0(
		arguments$report,
		'/cna_summary_ascat.tex'
		);

	# create symlinks for plots
	unlink(paste0(arguments$report,'/ascat_cna_landscape.png'));
	file.symlink(
		paste0(arguments$output,'/',
			generate.filename(arguments$project, 'ascat_cna_landscape','png')),
		paste0(arguments$report,'/ascat_cna_landscape.png')
		);

	unlink(paste0(arguments$report,'/ascat_metrics.png'));
	file.symlink(
		paste0(arguments$output,'/',
			generate.filename(arguments$project, 'ascat_metrics','png')),
		paste0(arguments$report,'/ascat_metrics.png')
		);
