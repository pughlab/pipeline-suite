...
sequences <- as.data.table(lapply(fasta$V4, function(x) {
	sapply(seq(from=1, to=nchar(x), by=2), function(i) { substr(x, i, i+1) } ) } ))
sequences <- as.data.frame(t(sequences))
sequences <- sequences[, -ncol(sequences)]
...
sequences2 <- as.data.frame(t(sequences2))

# fix levels for instances where not all contexts are present within a sequence
# necessary for later calls: sapply(sequences, table)
unique.contexts <- unique(unlist(sequences));
for (i in 1:ncol(sequences)) { sequences[,i] <- factor(sequences[,i], levels = unique.contexts); }

unique.contexts <- unique(unlist(sequences2));
for (i in 1:ncol(sequences2)) { sequences2[,i] <- factor(sequences2[,i], levels = unique.contexts); }
...
colnames(contexts2) <- seq(2, ncol(contexts2)*2, by = 2)
