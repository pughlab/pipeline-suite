...
adjust <- ceiling(10*log2(median(meanCov)+1));
...
## Generate QC report
QC_matrix <- data.frame(meanCov, sdCov, maxCov);
write.table(QC_matrix, file.path(outdir, paste0(id, "_cov_stats.txt")), sep = "\t");

rm(adjust, cutoff, maxCov, meanCov, sdCov, high_cov_regions, bamCov, QC_matrix);
